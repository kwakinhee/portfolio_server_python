...
os.environ.setdefault("PROCESS_TITLE", "configServer")
...
def start(application):
    try:
        ...
        configService.startService(application, __name__)
...
def stop():
    try:
        configService = ConfigService()
        if not configService.isRunniing():
            return

        glog.info("stop server")

        configService.stopService(__name__)
...
application = service.Application("configServer")
